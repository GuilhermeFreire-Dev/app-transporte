...
export default function Estado() {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/estado/${estado?.uf}`, {
      ...
      .then(async (res) => {
        if (res.ok) {
          ...
          let index = estados.findIndex((estado) => estado.uf === est.uf);
          estados[index] = est;
          setEstados(estados);
          setShowModal(false);
        } else {
  ...
  function handleInput(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    let v = ["icms_local", "icms_outro_uf"].includes(name)
      ? Number(value)
      : value;
    setEstado((prev) => ({
      ...prev,
          ...
          <Form>
            ...
            <Fieldset>
              ...
              <input
                name={"icms_local"}
                type="number"
                defaultValue={estado?.icms_local}
                onInput={handleInput}
                required
              ></input>
            </Fieldset>
            <Fieldset>
              ...
              <input
                name={"icms_outro_uf"}
                type="number"
                defaultValue={estado?.icms_outro_uf}
                onInput={handleInput}
                required
              ></input>
            </Fieldset>
